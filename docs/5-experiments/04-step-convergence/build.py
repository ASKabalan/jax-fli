...
spherical angular power spectra of the three integrators (``bf`` BullFrog, ``dkd`` DriftKickDrift,
``kdk`` DoubleKickDrift) run at ``--nb-steps ∈ {10, 20, 30, 40, 50}`` (steps 5/6 fail — the step count
must exceed the 10 shells). Everything else is fixed (2048³, nside 2048, 10 shells, BullFrog box, float64).

Figures (-> ``assets/``):
  fig01 / fig02   BullFrog, shells 0–4 / 5–9: per-shell C_ell at each step count + ratio to the 50-step run.
  fig03 / fig04   the same for DriftKickDrift.
  fig05 / fig06   the same for DoubleKickDrift.
  fig07           20- vs 30-step convergence of all three solvers at the near / mid / far shell:
                  C_ell + ratio to the Limber number-counts theory (× pixwin²(2048)).

Run from the repo root (CPU is fine; ~a minute, loads 15 precomputed spectra):
...
SOLVERS = {"bf": "BullFrog", "dkd": "DriftKickDrift", "kdk": "DoubleKickDrift"}
...
SOLVER_COLORS = {"bf": "tab:blue", "dkd": "tab:orange", "kdk": "tab:green"}
STEP7_STYLES = {20: "-", 30: "--"}  # fig07 distinguishes the two step counts by line style

root = snapshot_download(REPO, repo_type="dataset", local_files_only=True)

# -----------------------------------------------------------------------------
# Explicit per-file loads — every HF parquet (3 solvers × 5 step counts) is visible on its own line.
# -----------------------------------------------------------------------------
bf_s10_cat = Catalog.from_dataset(
    load_dataset("parquet", data_files=f"{root}/04-step-size/density_spectra/spectra_bf_s10.parquet", split="train")
)
bf_s20_cat = Catalog.from_dataset(
    load_dataset("parquet", data_files=f"{root}/04-step-size/density_spectra/spectra_bf_s20.parquet", split="train")
)
bf_s30_cat = Catalog.from_dataset(
    load_dataset("parquet", data_files=f"{root}/04-step-size/density_spectra/spectra_bf_s30.parquet", split="train")
)
bf_s40_cat = Catalog.from_dataset(
    load_dataset("parquet", data_files=f"{root}/04-step-size/density_spectra/spectra_bf_s40.parquet", split="train")
)
bf_s50_cat = Catalog.from_dataset(
    load_dataset("parquet", data_files=f"{root}/04-step-size/density_spectra/spectra_bf_s50.parquet", split="train")
)
...
cosmo = bf_s50_cat.cosmology[0]  # one fiducial cosmology shared by every run

# Solver -> step -> raw C_ell PowerSpectrum, assembled by hand from the explicit loads above.
spectra = {
    "bf": {
        10: bf_s10_cat.field[0],
        20: bf_s20_cat.field[0],
        30: bf_s30_cat.field[0],
        40: bf_s40_cat.field[0],
        50: bf_s50_cat.field[0],
    },
    "dkd": {
...
for _c in (bf_s10_cat, dkd_s50_cat, kdk_s50_cat):
...
# =============================================================================
# fig01–fig06 — per-solver step convergence: C_ell at each step count (top) + ratio to the 50-step run
# =============================================================================
def plot_step_convergence(solver, shell_idxs, stem):
    ...
    fig.suptitle(
        f"{SOLVERS[solver]} ({solver}) — per-shell angular $C_\\ell$ vs step count and the ratio to the 50-step run "
        f"(shells {shell_idxs[0]}–{shell_idxs[-1]}, nside={NSIDE})",
        y=1.0,
    )
...
# =============================================================================
# fig07 — 20 vs 30 steps, all three solvers, near / mid / far shell: C_ell + ratio to theory × pixwin²
# =============================================================================
def plot_three_solvers_near_mid_far(stem):
    ...
    fig.suptitle(
        "20- vs 30-step convergence of all three integrators at the near / mid / far shell, "
        rf"vs the Limber number-counts theory ($\times\,w_\ell^2$, nside={NSIDE})",
        y=1.0,
    )
    ...
    fig.legend(handles=handles, loc="upper center", ncol=7, fontsize=9, frameon=False, bbox_to_anchor=(0.5, 1.07))
    fig.tight_layout()
    savefig(ASSETS / stem, fig)
...
def main():
    set_style()
    plot_step_convergence("bf", range(0, 5), "fig01-bf-shells-0-4")
    plot_step_convergence("bf", range(5, 10), "fig02-bf-shells-5-9")
    plot_step_convergence("dkd", range(0, 5), "fig03-dkd-shells-0-4")
    plot_step_convergence("dkd", range(5, 10), "fig04-dkd-shells-5-9")
    plot_step_convergence("kdk", range(0, 5), "fig05-kdk-shells-0-4")
    plot_step_convergence("kdk", range(5, 10), "fig06-kdk-shells-5-9")
    plot_three_solvers_near_mid_far("fig07-three-solvers-near-mid-far")
