    ...
    def pixel_likelihood(observable):
        ...
        for idx, (observable_map, nz) in enumerate(zip(observable, nz_list)):
            if (
                ...
                loc = cut_input.scale_cut(config.ell_max, config.ell_taper_width, method=map2alm_method)
            elif do_scale_cut:
                loc = observable_map.scale_cut(config.ell_max, config.ell_taper_width, method=map2alm_method)
            else:
                loc = observable_map
