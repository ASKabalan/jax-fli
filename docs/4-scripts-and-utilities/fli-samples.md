...
Plus the shared **cosmology**, **simulation**, **integration**, **lensing**
(`--nz-shear --quadrature …`), **forward model** (`--lensing-output {convergence,shear,reduced_shear}`,
`--mask`, `--sigma-unobserved`, `--map2alm-method`) and **prior** (`--sample {cosmo,ic}`,
`--prior-omega-c …`) groups. The inference tutorials that consume these samples live under
