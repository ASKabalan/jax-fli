...
def _born_core_impl(
    ...
    broadcast_shape = (n_planes, *((1,) * (delta_planes.ndim - 1)))
    r_b = r.reshape(broadcast_shape)
    a_b = a.reshape(broadcast_shape)
    d_r = d_r.reshape(broadcast_shape)

    kappa_contributions = delta_planes * (d_r * r_b / a_b)
...
def _born_spherical(
    ...
    if source_kind == "distribution":
        ...
        kappa_maps = _born_core_impl(cosmo, lightcone_od.array, r_center, scale_factors, sources, density_plane_width)
    return kappa_maps
