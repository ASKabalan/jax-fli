...
# Four variants {kdk, dkd, bfa=BullFrog/a, bfd=BullFrog/D} × nb-steps {10,20,30,40,50}
# = 20 sims. float64.
...
# Time-stepping: kdk/dkd step in the scale factor a; BullFrog is run TWO ways — bfa (scale factor a)
# and bfd (growth factor D, better per arXiv:2409.19049) — so the stepping choice can be compared.
...
#   tag  --solver  --time-stepping   (bfa/bfd are both the BullFrog integrator, different stepping)
for RUN in "kdk kdk a" "dkd dkd a" "bfa bf a" "bfd bf D"; do
