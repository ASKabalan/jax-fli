...
--nside 2048 --scheme ngp --shells-per-file 1 --nb-shells $NB_SHELLS --shell-spacing a --time-stepping D --halo-multiplier 0.5 --enable-x64 --perf --iterations 3 --seed $SEED $COSMO"

for PO in cic tsc pcs; do
  for DECONV in "" "--deconvolution"; do
    if [ -n "$DECONV" ]; then tag="exp2_${PO}_deconv"; else tag="exp2_${PO}"; fi
    launch 16 4 64 1 00:45:00 -- $COMMON --paint-order "$PO" $DECONV \
