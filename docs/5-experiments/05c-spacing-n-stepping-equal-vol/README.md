...
The density figures (fig01–fig02) compare a coarse 10-shell lightcone to a much finer one at matched radial extent; the census (fig03–fig08) sweeps the full shell-count grid; and the Born convergence (fig09–fig20) contrasts the three shell quadratures and the two shell spacings.
...
**Equal-volume vs scale-factor spacing across shell count (fig14–fig20).** With the quadrature pinned at Gauss–Legendre (drift), the equal-volume and scale-factor ([05b](../05b-spacing-n-stepping-3bin/README.md)) Born convergence are compared for all three tomographic bins across a shell-count ladder — `N` = 10, 12, 16, 20, 25, 30, 40 (fig14–fig20 in increasing `N`). The top panel plots the `D_ℓ ≡ ℓ(ℓ+1) C_ℓ / 2π` power — solid = equal-volume, dashed = scale-factor, dash-dotted = the CosmoGrid reference itself. The judgement below is made against the **CosmoGrid reference itself** (the Stage-3 forecast κ at the `cosmo_172798` grid point, the grid cosmology closest to the runs), not against Limber: the middle panel shows `C_ℓ / CosmoGrid − 1` for both spacings against the acceptance band = the expected cosmology + nside-512-pixwin offset (`th_run/th_172798 − 1`, thin dotted — ≤ 2.5% in the window) ± **`√2 ×` the empirical CV of the 200 fiducial CosmoGrid permutations** (worst bin) — our run and the reference are independent single realizations, so a second CosmoGrid realization would live inside the band. The bottom panel condenses it to the **median of the bandpower ratios in the ℓ bands [30,100), [100,150), [150,200), [200,250), [250,300)** — one bar per spacing and bin, in the bin colours (solid = equal-volume, hatched = scale-factor) — with the same acceptance: the validity claim "good up to 250" is literal, the 200/250 bars sit inside the grey band or they do not. Spectra are bandpower-binned in linear bins of `nlb` = 32 multipoles. Both spacings sit on the reference around `ℓ ≈ 50–100` and roll off together below it and at small scales on the shared PM-resolution transfer (the ceiling any 2560³ run hits); the solid–dashed gap per colour is the spacing difference, and it **shrinks with shell count**. At `N` = 12 the equal-volume convergence carries the largest small-scale excess over 05b (its fat inner shell and floored-thin outer shells place the projected power differently from 05b's scale-factor shells); the gap narrows steadily through `N` = 20, 25, 30, and by `N` = 40 the two spacings overlay almost perfectly — the equal-volume geometry has converged onto the scale-factor result. Bin 1 sits lowest throughout, its low-`z` sources inside the fat inner ball. 05b was only run with the midpoint quadrature, but for its thin scale-factor shells midpoint and Gauss–Legendre agree to **< 0.2%** on the total per-bin lensing weight (individual shells deviate but cancel), so its midpoint spectra stand in for Gauss–Legendre here. The headline is the contrast with the midpoint runs of fig10: with the exact quadrature the two spacings differ modestly and converge with shell count, rather than the midpoint's 2–3× — the spacing was never the large lensing problem, the quadrature was.
