...
if STAGE == "check":
    ...
    print(
        f"rfft path stays real              : max|imag-equivalent| = {np.abs(mine.imag).max() if np.iscomplexobj(mine) else 0.0:.3e}"
    )
...
cosmo = jc.Cosmology(
    Omega_c=_c[0], Omega_b=_c[1], h=_c[2], n_s=_c[3], sigma8=_c[4], w0=_c[5], wa=_c[6], Omega_k=_c[7], Omega_nu=_c[8]
)
...
for tag, W, n, (sa, ca, pa) in (("target", WT, NT, (s22a, c22a, pk2a)), ("source", WS, NS, (s11a, c11a, pk1a))):
...
cols = dict(
    kcen=kcen, s11b=s11b, s22b=s22b, s12b=s12b, cb=cb, s11a=s11a, c11a=c11a, pk1a=pk1a, s22a=s22a, c22a=c22a, pk2a=pk2a
)
