...
# 1024³ tops out at 256 GPUs. The gradient roughly DOUBLES the forward working set; the per-GPU ceiling is
# unified with Exp 11's float64 300³ (a conservative cap the reversible kdk backsolve stays well under).
...
# Forward model differentiated: final-version forward model — PM + lightcone painting with CIC (no force-window
# deconvolution), RBF 0.8-pixel spherical smoothing, drift on the lightcone, 20 shells. 30 steps, reversible
# kdk. nside tracks the smallest global-mesh dimension, set per launch below (nside = M for the strong-scaling
# grid, 256 for the weak slab).
GRAD_COMMON="--sim-mode pm --solver kdk --nb-steps 30 --paint-order cic --scheme rbf_neighbor --kernel-width-pixels 0.8 \
--nb-shells 20 --drift-on-lightcone --enable-x64 --perf --iterations 5 --seed $SEED $COSMO"
...
GRADS="reverse:rev checkpointed_4:ckpt4 checkpointed_8:ckpt8 checkpointed_16:ckpt16 checkpointed_20:ckpt20 checkpointed_30:ckpt30"   # O(1)-memory backsolve + five shell-scan checkpoint counts (20 and 30 are both ≥ 20 shells ⇒ store-all, i.e. the same program; build.py plots only ckpt30)
...
for gradspec in $GRADS; do
  ...
  for M in 1024; do
    for g in $GPUS; do
      ...
      launch "$nodes" 4 "$px" "$py" 00:30:00 -- $GRAD_COMMON --grad "$grad" --nside $M --mesh-size $M $M $M --box-size $BOX5 \
